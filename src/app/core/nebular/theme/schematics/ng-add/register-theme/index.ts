...
export function registerPrebuiltTheme(options: Schema) {
  return (tree: Tree) => {
    ...
    const themePath = `./node_modules/@core/nebular/theme/styles/prebuilt/${options.theme}.css`;
...
function noNebularThemeIncluded(targetOptions: any, stylesPath: string): boolean {
  ...
  const hasOtherTheme = existingStyles.find((s: any) => s.includes('@core/nebular/theme/styles/prebuilt'));
