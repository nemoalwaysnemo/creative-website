...
import { AbstractControl, AsyncValidatorFn, ValidatorFn, Validators, NG_VALIDATORS, NG_ASYNC_VALIDATORS } from '@angular/forms';
import { DynamicFormControlModel } from '../model/dynamic-form-control.model';
import { DynamicValidatorDescriptor, DynamicValidatorsConfig } from '../model/misc/dynamic-form-control-validation.model';
...
export class DynamicFormValidationService {
  ...
  private getValidatorFn(validatorName: string, validatorArgs: any = null,
    ...
    if (Validators.hasOwnProperty(validatorName)) { // Built-in Angular Validators
      ...
      if (this.dynamicValidators && this.dynamicValidators.has(validatorName)) {
        validatorFn = this.dynamicValidators.get(validatorName);
      } else if (validatorsToken) {
        validatorFn = validatorsToken.find(_validatorFn => _validatorFn.name === validatorName);
      }
    }
  ...
  getAsyncValidator(validatorName: string, validatorArgs: any = null): AsyncValidatorFn {
    return this.getValidatorFn(validatorName, validatorArgs, this.asyncValidators) as AsyncValidatorFn;
  }
  ...
  getAsyncValidators(asyncValidatorsConfig: DynamicValidatorsConfig): AsyncValidatorFn[] {
    return this.getValidatorFns(asyncValidatorsConfig, this.asyncValidators) as AsyncValidatorFn[];
  }
