...
@Component({
  selector: 'tbwa-recommend-brand-search-form',
...
export class SearchFormComponent extends BaseAutoSearch {
  ...
  private onMessageChange() {
    ...
    const subscription = this.brandSerice.onChangeMessage().pipe(map((message) => `["${message.brand.join('", "')}"]`)).subscribe((brand) => {
      this.params.the_loupe_main_brand_any = brand;
      this.setFormValues(deepExtend({ the_loupe_main_brand_any: brand }, this.queryParamsService.getCurrentQueryParams()));
      this.onSearch();
  ...
  private setFormValues(queryParams: any): void {
    ...
    params = selectObjectByKeys(params, ['ecm_fulltext', 'pageSize', 'currentPageIndex', 'aggregates', 'ecm_path', 'the_loupe_main_brand_any']);
